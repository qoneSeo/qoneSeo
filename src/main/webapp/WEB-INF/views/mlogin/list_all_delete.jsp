...
<c:import url="/menu/top.do" />
 ...
 <div id='panel_delete' style='padding: 10px 0px 10px 0px; background-color: #F9F9F9; width: 100%; text-align: center;'>
    <form name='frm_delete' id='frm_delete' method='post' action='./delete.do'>
        <input type="hidden" name="mloginno" value="${param.mloginno }"> <%-- 삭제할 카테고리 번호 --%>

        <c:choose>
            <c:when test="${count_by_mloginno >= 1 }"> <%-- 자식 레코드가 있는 상황 --%>
                <div class="msg_warning">
                    관련 자료 ${count_by_mloginno } 건이 발견되었습니다.<br>
                    관련 자료를 모두 삭제하시겠습니까?
                </div>
                <label>관련 카테고리 이름</label>:  ${mloginVO.name } 
    ...
    <c:forEach var="MloginVO" items="${list }" varStatus="info">
      <c:set var="mloginno" value="${MloginVO.mloginno }"/>
        <tr>
          ...
          <td class="td_bs">${MloginVO.logindate }</td>
          <td class="td_bs">
